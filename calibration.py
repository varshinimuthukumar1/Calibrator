...
from utils import get_correspondences, match_points_stereo, load_calibration_data, rotate_yaw_pitch_roll

import numpy as np
import cv2
imageSize = (2280,3648)

def vis_image(img, img_pts):
...
def undistort(img, mtx, dist, vis=True):
    img = cv2.rotate(img, cv2.ROTATE_90_COUNTERCLOCKWISE)
    img = cv2.resize(img, (2280,3648))
    
    map1, map2 = cv2.fisheye.initUndistortRectifyMap(mtx, dist, np.eye(3), mtx, imageSize, cv2.CV_16SC2)
...
def get_intrinsics(img_pts, obj_pts,save=True):
    ...
    retval, mtx, dist, _,_ = cv2.fisheye.calibrate(
        obj_pts, img_pts, (2280,3648), mtx, dist, None, None,calibration_flags,
...
def get_extrinsics(img_pts, obj_pts, mtx, dist, rvecs=None, tvecs=None):
    fx = mtx[0, 0]
    fy = mtx[1, 1]
    cx = mtx[0, 2]
    cy = mtx[1, 2]
...
def calibrate_camera(vis:bool = True):
    ...
    matched_img_points_left, matched_obj_points_l = get_3dto2d_correspondences(imgpoints_l["annotated_points"],objpoints, img_path=TEST_IMAGE_LEFT)
    matched_img_points_right, matched_obj_points_r = get_3dto2d_correspondences(imgpoints_r["annotated_points"],objpoints, img_path=TEST_IMAGE_RIGHT)

    ## Get the intrinsics
    mtx, dist = get_intrinsics([matched_img_points_left,matched_img_points_right],[matched_obj_points_l,matched_obj_points_r])
    ...
    for point in undistort_pts:
        ...
        if point[0][0] > 0 and point[0][0] < 2280  and point[0][1] > 0 and point[0][1] < 3648:
            img_undist = cv2.circle(img_undist, (int(point[0][0]),int(point[0][1])), 25, (0, 255, 0), -1)
    img_undist = cv2.resize(img_undist, DISPLAY_SIZE)
    cv2.imshow('Remapped to undistorted image', img_undist)
    cv2.waitKey(0)

    ## Get the extrinsics
    rvec, tvec,pts_temp = get_extrinsics(matched_img_points_left,matched_obj_points_l,mtx,dist)
    ...
    for point in undistort_pts:
        point[0][0],point[0][1] = point[0][0] * fx + cx, point[0][1]* fy + cy
        if point[0][0] > 0 and point[0][0] < 2280  and point[0][1] > 0 and point[0][1] < 3648:
            img_undist_test = cv2.circle(img_undist_test, (int(point[0][0]),int(point[0][1])), 25, (255, 0, 0), -1)
