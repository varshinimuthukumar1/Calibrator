...
def stereo_calibrate(
    matched_obj_points:np.array, matched_img_points_right:np.array, matched_img_points_left:np.array, mtx:np.array, dist:np.array
):
    ...
    u_matched_img_points_right = cv2.fisheye.undistortPoints(
        matched_img_points_right, mtx, dist
    )
    u_matched_img_points_left = cv2.fisheye.undistortPoints(
        matched_img_points_left, mtx, dist
    )
    # Ensure there's an equal number of objectPoints, imagePointsLeft, and imagePointsRight
    assert (
        len(u_matched_obj_points)
        == len(u_matched_img_points_left)
        == len(u_matched_img_points_right)
    ), "Mismatch in the number of views"

    # Ensure each set of points has a consistent number of points across left and right images
    for obj, img_l, img_r in zip(
        u_matched_obj_points, u_matched_img_points_left, u_matched_img_points_right
    ):
        assert (
            obj.shape[0] == img_l.shape[0] == img_r.shape[0]
        ), "Mismatch in the number of points within a view"
    ...
    assert (
        matched_obj_points.dtype == np.float32
    ), "Object points must be of type np.float32"
    assert matched_obj_points.shape[1:] == (
        1,
        3,
    ), "Object points must have a shape of (-1, 1, 3)"
    ...
    retval, K1, D1, K2, D2, R, T, E, F = cv2.stereoCalibrate(
        [u_matched_obj_points],
        [u_matched_img_points_left],
        [u_matched_img_points_right],
        K1,
        D1,
        K2,
        D2,
